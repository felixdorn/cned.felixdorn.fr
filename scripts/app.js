    ...
    fetch('/links.json').then(res => res.json()).then(links => {
        Object.keys(links).sort().filter(name => {
            ...
            el.innerHTML = `<a href="${link}" class="flex justify-between items-start sm:items-stretch rounded-lg p-4 transition ease-in-out duration-150 bg-gray-800 border-t border-gray-700 focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 focus:ring-offset-gray-800 focus:outline-none">
                ...
                <button type="button"
                        class="transition ease-in-out duration-150 bg-gradient-to-t from-purple-500 to-purple-400 px-6 py-2 rounded-lg font-semibold text-white hover:from-purple-400 hover:to-purple-500 focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 ring-offset-gray-700 focus:outline-none">
                    Continuer
                </button>
            </a>`
